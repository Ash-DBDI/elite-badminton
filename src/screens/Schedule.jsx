...
const numInput = {
  width: '80px', height: '64px', padding: '4px',
...
function ScoreControl({ label, value, onChange }) {
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      <div style={{ fontSize: '10px', color: 'var(--muted)', fontWeight: 600, marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
        {label}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
        <button onClick={() => onChange(String(Math.max(0, num - 1)))}
          style={{ ...pmBtn, background: 'var(--surface2)', color: 'var(--text)' }}>{'\u2212'}</button>
        <input type="number" inputMode="numeric" min="0" max="30"
          value={value} onChange={e => onChange(e.target.value)}
          style={numInput} />
        <button onClick={() => onChange(String(Math.min(30, num + 1)))}
          style={{ ...pmBtn, background: 'var(--gold-dim)', color: 'var(--gold)', border: '1px solid var(--gold-border)' }}>{'\uFF0B'}</button>
      </div>
    </div>
  )
}
...
export default function Schedule() {
  ...
  async function handleSubmitScore() {
    ...
    try {
      await supabase.from('games').update({
        team_a_score: a, team_b_score: b, status: 'completed', locked: true, completed_at: new Date().toISOString()
      ...
      for (const pid of fourIds) {
        ...
        await supabase.from('session_players').update({
          ...
          points_scored: sp.points_scored + (inA ? a : b),
          points_conceded: sp.points_conceded + (inA ? b : a)
        }).eq('id', sp.id)
      }
      ...
      for (const pid of fourIds) {
        ...
        const inA = [currentGame.team_a_player1, currentGame.team_a_player2].includes(pid)
        const won = inA ? teamAWon : !teamAWon
        const newR = updatedRating(p.skill_rating, inA ? expA : 1 - expA, won ? 1 : 0)
        const newStreak = won ? (p.current_streak || 0) + 1 : 0
        await supabase.from('players').update({
          skill_rating: newR, total_games: (p.total_games || 0) + 1,
          total_wins: (p.total_wins || 0) + (won ? 1 : 0), total_losses: (p.total_losses || 0) + (won ? 0 : 1),
          total_points_scored: (p.total_points_scored || 0) + (inA ? a : b),
          total_points_conceded: (p.total_points_conceded || 0) + (inA ? b : a),
          current_streak: newStreak, best_streak: Math.max(p.best_streak || 0, newStreak)
        }).eq('id', pid)
      }
      ...
      if (ap.length >= 4) {
        ...
        if (pc > 0) {
          const rs = reshuffleRemaining(ap, updComp, pc, currentGame.game_number + 1)
      ...
      const allComp = [...completed, { ...currentGame, team_a_score: a, team_b_score: b }].sort((x, y) => x.game_number - y.game_number)
      for (const pid of fourIds) {
        const pg = allComp.filter(g => [g.team_a_player1, g.team_a_player2, g.team_b_player1, g.team_b_player2].includes(pid))
        let s = 0
        for (const g of pg) { const iA = [g.team_a_player1, g.team_a_player2].includes(pid); if (iA ? g.team_a_score > g.team_b_score : g.team_b_score > g.team_a_score) s++; else s = 0 }
        if (s >= 3) { const { data: ex } = await supabase.from('badges').select('id').eq('player_id', pid).eq('session_id', activeSession.id).eq('badge_type', 'HOT_HAND'); if (!ex?.length) await supabase.from('badges').insert({ player_id: pid, session_id: activeSession.id, badge_type: 'HOT_HAND' }) }
      }

      setScoreA(''); setScoreB(''); setScoreError('')
      await loadActiveSession(); await loadPlayers()
  ...
  async function handleDeleteGame(game) {
    if (game.status === 'completed') {
      // Decrement session_player stats for the 4 players
      ...
      for (const pid of fourIds) {
        const sp = spList.find(s => s.player_id === pid)
        if (!sp) continue
        const inA = [game.team_a_player1, game.team_a_player2].includes(pid)
        const won = inA ? aWon : !aWon
        await supabase.from('session_players').update({
          games_played: Math.max(0, sp.games_played - 1),
          games_won: Math.max(0, sp.games_won - (won ? 1 : 0)),
          points_scored: Math.max(0, sp.points_scored - (inA ? game.team_a_score : game.team_b_score)),
          points_conceded: Math.max(0, sp.points_conceded - (inA ? game.team_b_score : game.team_a_score))
        }).eq('id', sp.id)
      }
    }
  ...
  async function handleEditSave(game) {
    setEditError('')
    const err = validate(editA, editB)
    ...
    for (const pid of fourIds) {
      ...
      if (!sp) continue
      const inA = [game.team_a_player1, game.team_a_player2].includes(pid)
      const oldWon = inA ? oldAWon : !oldAWon
      await supabase.from('session_players').update({
        games_played: Math.max(0, sp.games_played - 1),
        games_won: Math.max(0, sp.games_won - (oldWon ? 1 : 0)),
        points_scored: Math.max(0, sp.points_scored - (inA ? game.team_a_score : game.team_b_score)),
        points_conceded: Math.max(0, sp.points_conceded - (inA ? game.team_b_score : game.team_a_score))
      }).eq('id', sp.id)
    }
    ...
    for (const pid of fourIds) {
      const sp = (freshSp || []).find(s => s.player_id === pid)
      if (!sp) continue
      const inA = [game.team_a_player1, game.team_a_player2].includes(pid)
      const won = inA ? newAWon : !newAWon
      await supabase.from('session_players').update({
        games_played: sp.games_played + 1,
        games_won: sp.games_won + (won ? 1 : 0),
        points_scored: sp.points_scored + (inA ? a : b),
  ...
  async function handleRegenerate() {
    ...
    if (rs.length > 0) await supabase.from('games').insert(rs.map(g => ({
  ...
  function GameCard({ game, isCurrent }) {
    ...
    return (
      <div style={{
        ...
        padding: isCurrent ? '20px' : '14px 16px', marginBottom: '10px'
      }}>
        {/* Header */}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div style={{ flex: 1, textAlign: 'center' }}>
            {[game.team_a_player1, game.team_a_player2].map(id => {
              const p = P(id)
              return <div key={id} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px', justifyContent: 'center' }}>
        ...
        {/* Admin buttons on completed games */}
        {isDone && isAdmin && !isEditing && (
          <div style={{ display: 'flex', gap: '6px', marginTop: '10px' }}>
            <button onClick={() => { setEditingGame(game.id); setEditA(String(game.team_a_score)); setEditB(String(game.team_b_score)); setEditError('') }}
              style={{ flex: 1, padding: '8px', borderRadius: '8px', background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontSize: '11px', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
              {'\u{270F}\u{FE0F}'} Edit Score
            </button>
            <button onClick={() => setConfirmDelete(game)}
              style={{ flex: 1, padding: '8px', borderRadius: '8px', background: 'var(--red-dim)', border: '1px solid var(--red)', color: 'var(--red)', fontSize: '11px', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
              {'\u{1F5D1}\u{FE0F}'} Delete Game
            </button>
          </div>
        )}

        {/* Admin edit score form */}
        {isDone && isEditing && (
          <div style={{ marginTop: '14px' }}>
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', alignItems: 'center', marginBottom: '8px' }}>
              ...
              <div style={{ paddingTop: '22px', fontSize: '16px', fontWeight: 800, color: 'var(--muted)' }}>{'\u2013'}</div>
        ...
        {/* Admin remove pending game */}
        {isPending && isAdmin && (
          <button onClick={() => setConfirmDelete(game)}
            style={{ width: '100%', marginTop: '10px', padding: '8px', borderRadius: '8px', background: 'var(--red-dim)', border: '1px solid var(--red)', color: 'var(--red)', fontSize: '11px', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
            {'\u{1F5D1}\u{FE0F}'} Remove from schedule
          </button>
        )}

        {/* Score input for active game — combined type + buttons */}
        {isActive && isCurrent && (
          <div style={{ marginTop: '16px' }}>
            ...
            <button onClick={handleSubmitScore} disabled={submitting} style={{
              width: '100%', padding: '16px', minHeight: '56px', borderRadius: '12px', marginTop: '12px',
              background: submitting ? 'var(--surface2)' : 'var(--gold)',
              border: 'none', color: '#111', fontWeight: 700, fontSize: '16px',
        ...
        <div onClick={() => setConfirmDelete(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300, padding: '16px' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', borderRadius: '16px', border: '1px solid var(--border)', padding: '24px', maxWidth: '300px', width: '100%', textAlign: 'center' }}>
            <div style={{ fontSize: '16px', fontWeight: 600, marginBottom: '8px' }}>
              {confirmDelete.status === 'completed' ? `Delete Game ${confirmDelete.game_number} and its results?` : `Remove Game ${confirmDelete.game_number}?`}
            </div>
            <div style={{ fontSize: '13px', color: 'var(--muted)', marginBottom: '20px' }}>
              {confirmDelete.status === 'completed' ? 'Player stats for this game will be reversed.' : 'This game will be removed from the schedule.'}
            </div>
